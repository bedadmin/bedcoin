...
static std::vector<RPCResult> MempoolEntryDescription() { return {
    ...
    RPCResult{RPCResult::Type::NUM, "size", "(DEPRECATED) same as vsize. Only returned if bedcoind is started with -deprecatedrpc=size\n"
...
static UniValue getblock(const JSONRPCRequest& request)
{
    ...
    uint256 hash;
    try {
        hash = uint256(ParseHashV(request.params[0], "blockhash"));
    } catch (...) {
        nHeight = std::stol(request.params[0].get_str());
        if (nHeight < 0 || nHeight > ::ChainActive().Height())
            throw JSONRPCError(RPC_INVALID_PARAMETER, "Block height out of range");
    }
                        ...
                        {RPCResult::Type::OBJ, "scriptPubKey", "",
                            {
                                ...
                                {RPCResult::Type::ARR, "addresses", "array of bedcoin addresses",
...
UniValue dumptxoutset(const JSONRPCRequest& request)
{
...

    {
        ...
        //
        // See discussion here:
        //   https://github.com/bedcoin/bedcoin/pull/15606#discussion_r274479369
        //
        LOCK(::cs_main);
