...
const char * const BITCOIN_CONF_FILENAME = "bedcoin.conf";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bedcoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bedcoin
    // Mac: ~/Library/Application Support/Bedcoin
    // Unix: ~/.bedcoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bedcoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bedcoin";
#else
    // Unix
    return pathRet / ".bedcoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    if (copyright_devs.find("Bedcoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bedcoin Core developers";
    }
